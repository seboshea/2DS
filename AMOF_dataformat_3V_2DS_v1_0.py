...
# Generate a .nc file for the 2DS part of the 3V that meets CEDA criteria
...
#######################
#Save data as netcdf
Nc_filename='NCAS-3V-2DS_faam_'+FlightDate.strftime("%Y%m%d")+'_v'+vnumber+'_r'+str(rnumber)+'_'+FlightNumber+'_particle-size-distribution.nc'
print(Nc_filename)
NcFile= netCDF4.Dataset(DataPath+Nc_filename,'w', format='NETCDF4') #'w' stands for write

# Global attributes
NcFile.conventions ='CF-1.6'
NcFile.source = 'NCAS 3V-2DS'
NcFile.instrument_manufacturer = 'Spec Inc., USA'
NcFile.instrument_model = '3V-2DS'
...
NcFile.acknowledgement='To discuss use and interest in this data please contact Sebastian OShea (sebastian.oshea@manchester.ac.uk).'
...
NcFile.comment = 'Data processing was performed to derive size segregated number concentrations as in OShea et al. (https://doi.org/10.5194/amt-2020-265). Particle size has been calculated as the mean particle extent parallel and perpendicular to the probe optical array. Particles with inter-arrival times <'+str(IAT_threshold)+'s have been rejected. The PSD <'+str(ThresholdSize)+' um has been calculated using stereo pairs. Pairs of images were identified as stereo if time difference between them <'+str(ColocationThreshold)+'s and diameter y difference is <='+str(ThresholdDeltaDiameterY)+'. Particles in contact with the edge of the sample array have been rejected. Other aircraft variables have been extracted from the file '+CoreFileName+'. The instrument counting uncertainty for each time and size bin is given by (100% / N^(1/2)), where N is the number of counts in the bin.'
